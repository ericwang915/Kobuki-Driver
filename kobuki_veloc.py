...
class Ctrl_Motion:
    ...
    def _init_params(self):
        ...
        self.fw.truncate()
        self.k_l=0.4
        self.L_1=20*np.eye(3)
        self.L_2=15*np.eye(3)
        self.L_3=5*np.eye(3)
        self.z=np.zeros((2,1))
        self.z0=np.zeros((2,1))
        self.etadot=np.zeros((3,1))
        self.eta=0.1*np.eye(3,1)
        self.toc=0
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_q:
                    ...
                    pygame.exit()
        toc1= self.toc
        self.toc = rospy.get_time()-self.t0
        self.eta=self.eta+self.etadot*(self.toc-toc1)
        if self.RunFlag==1:
            xr=self.scale*self.toc
            dxr=self.scale
            yr=0.2*np.sin(self.scale*self.toc)
            dyr=0.2*self.scale*np.cos(self.scale*self.toc)
            thetar=np.arctan(dyr/dxr)
            dthetar=(1/(1+np.power(0.2*np.cos(self.scale*self.toc),2))*(-0.2*np.sin(self.scale*self.toc)*self.scale))
            wr=dthetar
            vr=dxr/np.cos(thetar)
            qrdot=np.array([[dxr],[dyr],[dthetar]])
            qr=np.array([[xr],[yr],[thetar]])
            q=np.array([[self.odomx],[self.odomy],[self.yaw]])
            self.fw.write(str(self.toc)+'   '+str(qr[0][0])+' '+str(qr[1][0])+' '+str(qr[2][0])+'    '+str(q[0][0])+' '+str(q[1][0])+' '+str(q[2][0])+' '+str(self.z[0][0])+' '+str(self.z[1][0])+'\n')
            ctheta=np.cos(self.yaw)
            stheta=np.sin(self.yaw)
            Te=np.array([[ctheta,-stheta,0],[-stheta,ctheta,0],[0,0,1]])
            e=np.dot(Te, qr-q)
            Fe=np.array([[vr*np.cos(e[2][0])],[vr*np.sin(e[2][0])],[wr]])
            Ge=np.array([[-1,e[1][0]],[0,-e[0][0]],[0,-1]])
            eta=np.array([[0.01],[0.01],[0.01]])
            deltaz=self.z-self.z0
            if LA.norm(eta)>=0.001:
                self.etadot=np.dot(-self.L_1,eta)-LA.pinv(np.transpose(eta))*np.fabs(np.dot(np.dot(np.transpose(e),Ge),deltaz))
            else:
                self.etadot=np.zeros(3,1)

            z0=np.dot(LA.pinv(Ge),(-np.dot(self.L_2,e)-np.dot(self.L_3,eta)-Fe))
            self.z=2*self.k_l/np.pi*np.arctan(z0)
            print self.z
            self.twist.linear.x=self.z[0][0]
            self.twist.angular.z=self.z[1][0]
